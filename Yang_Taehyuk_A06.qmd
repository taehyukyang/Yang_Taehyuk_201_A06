...
[https://github.com/taehyukyang/Yang_Taehyuk_201_A06](https://github.com/taehyukyang/Yang_Taehyuk_201_A06)
...
```{r}
indexing_points <- player_info[[2]][[4]]
name_points <- player_info$LJ$points
...
for(player in (names(player_info))){
  player_info[[player]] <- c(player_info[[player]], height = player_heights[player])
}
...
```{r}
pts_per_game <- function(player){
  player$points / player$games_played
}

points_per_game <- lapply(player_info, pts_per_game)
points_per_game
```
### Part d (1 point)
...
```{r}
two_decimal_points <- function(points){
  round(points, 2)
}

rounded_points <- lapply(points_per_game, two_decimal_points)
rounded_points
```
## Question 3 (3 points)
...
rounded_matrix <- apply(mat_rand, MARGIN = 1:2, rounding)
rounded_matrix
```
## Question 4 (9 points)
...
for(temp in colnames(max_temps)[-1]){
  average_temp[[temp]] <- mean(max_temps[[temp]])
}
...
```{r}
median_annual_temps <- apply(colorado_temps[, c("jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec")], 1, median)

colorado_temps$median_annual_temps <- median_annual_temps

max_median_index <- which.max(median_annual_temps)
min_median_index <- which.min(median_annual_temps)

max_median_temp <- colorado_temps[max_median_index, ]
min_median_temp <- colorado_temps[min_median_index, ]

print(paste0("The maximum median annual temperature of ", max_median_temp$median_annual_temps, " occurred in ", max_median_temp$year, " and the minimum median annual temperature of ", min_median_temp$median_annual_temps, " occurred in ", min_median_temp$year, "."))
